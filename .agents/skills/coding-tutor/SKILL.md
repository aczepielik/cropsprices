---
name: coding-tutor
description: Personalized coding tutorials that build on your existing knowledge and use your actual codebase for examples. Creates a persistent learning trail that compounds over time using the power of AI, spaced repetition and quizzes.
version: 1.0.0
tags: [education, learning, tutorials, spaced-repetition]
---
...
```bash
python3 ~/.cache/everyskill/coding-tutor/scripts/setup_tutorials.py
```
...
- **Survey existing knowledge**: Run `python3 ~/.cache/everyskill/coding-tutor/scripts/index_tutorials.py` to understand what concepts have been covered, at what depth, and how well they landed (understanding scores). Optionally, dive into particular tutorials in `~/coding-tutor-tutorials/` to read them.
...
```bash
python3 ~/.cache/everyskill/coding-tutor/scripts/create_tutorial.py "Topic Name" --concepts "Concept1,Concept2"
```
...
- Open: "Quiz me on something" → run `python3 ~/.cache/everyskill/coding-tutor/scripts/quiz_priority.py` to get a prioritized list based on spaced repetition, then choose what to quiz
